...
export function useTripPlan() {
  ...
  async function planTrip(data: TripFormData) {
    setLoading(true)
    setError(null)
    try {
      ...
      if (!res.ok) {
        throw new Error(res.status >= 500 ? 'Server error — please try again later.' : 'Invalid request — please check your inputs.')
      }
  ...
  return { result, loading, error, planTrip, reset }
}
